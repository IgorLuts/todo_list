class TasksController < ApplicationController
  ...
  before_action :set_task, only: [:show, :edit, :update, :destroy, :set_position]
  ...
  def set_position
    @task.set_list_position(task_params[:position].to_i + 1)
    head :no_content
  end
  ...
  private
    ...
    def task_params
      params.require(:task).permit(:name, :completed, :deadline, :position)
    end
